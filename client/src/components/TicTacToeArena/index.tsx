...
export function TicTacToeArena({ 
  ...
  return (
    <div className="w-full flex flex-col items-center gap-8 relative">
            ...
            >
              ...
              {isPlayer ? (
                <div className="flex flex-col gap-4 w-full min-w-[240px]">
                  ...
                  >
                    {amIReady ? "WAITING FOR OPPONENT..." : "INITIALIZE NEXT CYCLE"}
                  </Button>
      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-[360px] aspect-square relative p-2 bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
        {/* HUD Scanner overlay for the grid */}
        <div className="absolute inset-0 z-0 pointer-events-none opacity-5">
           <div className="absolute inset-0 bg-[length:20px_20px] bg-[linear-gradient(to_right,#fff_1px,transparent_1px),linear-gradient(to_bottom,#fff_1px,transparent_1px)]"></div>
        </div>
        ...
        {board.map((cell: string | null, i: number) => (
          <motion.div
            ...
            className={`
              relative flex items-center justify-center text-5xl font-black rounded-xl border border-white/5 transition-all
              ${!cell && isMyTurn && isPlayer && !winner ? 'cursor-pointer hover:border-primary/40' : 'cursor-default'}
              ${cell === 'X' ? 'text-primary' : 'text-destructive'}
              ${winner && board.some((_:any,idx:any) => i === idx) ? 'bg-white/[0.02]' : ''}
            `}
          >
            ...
            {/* Cell Coordinate HUD */}
            <span className="absolute bottom-2 right-2 text-[8px] font-mono text-white/10 uppercase font-bold">
               N-0{i}
            </span>
          </motion.div>
        ))}
      ...
      <div className="flex items-center gap-4 w-full max-w-[360px]">
         <div className={`flex-1 p-4 rounded-xl border transition-all ${isMyTurn && !winner ? 'bg-primary/10 border-primary/40 shadow-[0_0_20px_rgba(6,182,212,0.1)]' : 'bg-white/5 border-white/5'}`}>
            <span className="text-[10px] font-black uppercase tracking-widest block mb-1 text-white/30">Current Turn</span>
            <span className={`text-sm font-black uppercase tracking-widest ${isMyTurn ? 'text-primary' : 'text-white/60'}`}>
